...
// Clock
#define TICK_WAIT_TIME_SECONDS \
    5 // wait 5 seconds between scans
...
#define MAX_CONSEC \
    6 // do not grow consec path this value to
...
inline void tick_whitelist(whitelist *wl) { ++wl->ticks; }
...
bool is_whitelisted(whitelist *wl, uint32_t pid, uint32_t *idx) {
    *idx = hash(pid);
    ...
    for (uint32_t probe_count = 0; probe_count < WL_MAP_SIZE; probe_count++) {
        ...
        bool expired =
            is_expired(wl->ticks, wl->map[*idx].born_at, wl->map[*idx].ttl);
